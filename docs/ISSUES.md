...
- [ ] P1: 桁区切り表示値の数値変換を修正する
  - [ ] `toNumber` で `,` を除去してから数値化する
  - [ ] 空文字や不正値は `null` 扱いを維持する
  - [ ] 確認: `1,234 + 1 = 1,235` が成立する
  - [ ] 確認: `MS` / `M+` / `M-` が `1,234` 表示時にも動作する

- [ ] P2: `clearAll` 実行時に設定値を保持する
  - [ ] `taxRate` / `precision` / `grouping` / `scientific` を引き継ぐ
  - [ ] 計算コンテキストのみ初期化する
  - [ ] 確認: `precision=6` に設定後 `C` 押下で `6` が保持される
  - [ ] 確認: `grouping` / `scientific` / `taxRate` も `C` で変化しない

- [ ] P2: 式モードの小数点入力制御をオペランド単位に変更する
  - [ ] `displayValue` 全体ではなく、現在オペランドのみで `.` 重複判定する
  - [ ] 演算子区切りで現在オペランドを抽出する
  - [ ] 確認: `1.2+3.4` / `10.05*2.1` が入力できる
  - [ ] 確認: `1..2` / `3.4.5` は拒否される
