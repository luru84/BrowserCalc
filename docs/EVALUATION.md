...
- モード切替（逐次 ↔ 式評価）  
  - localStorage にモード選好がない初回起動では式評価モードとなり、状態表示が `式評価` となる。
  - 入力順モードへ切り替えると状態表示が `入力順` となり、リロード後も選択が保持される。
  - 式モードで `1 + 2 * 3 =` → 7（演算子優先）。エラー時は式が保持され、`C` で復帰。括弧は現行スコープ外。
  - `grouping=ON` で式モード `1234 =` 後に `+ 1 =` が成功し、`Invalid character` にならない。
- キーボード操作  
  - 数字/演算子 `+ - * /`、Enter/`=` で計算、Backspace で削除、`Esc`/`c` で全クリア、`%` でパーセント。ブラウザ標準ショートカットを阻害しないこと。
- エラー/制約  
  - `1 / 0 =` でエラー表示、`C` で復帰。桁上限（整数12桁）を超える入力は無視またはクリップされる。
  - 設定を変更した状態でエラーを発生させ、`CE` で復帰しても税率/精度/桁区切り/科学表記が保持される。
- 永続化  
  - テーマ、税率、表示設定、モード選好がリロード後も保持される。履歴はリロードで消える。
- 履歴再利用  
  - 逐次計算途中で履歴を再利用しても、前の `accumulator/pendingOperator/recentOperand` が混入しない。
- 設定導線（ハンバーガーメニュー）  
  - 初期表示は設定パネル閉。ハンバーガーボタンで開閉できる。
  - `Esc`、閉じるボタン、背景クリックで閉じられる。
  - `aria-expanded` / `aria-controls` / `aria-label` が適切に設定される。
- テーマ/レスポンシブ  
