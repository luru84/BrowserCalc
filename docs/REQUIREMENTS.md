...
- 入力モード切替:
  - 逐次計算モード（現在の挙動）と、括弧を含む式評価モードを切替可能にする。
  - 初期デフォルトは逐次計算モード。
  - モード別仕様:
    - 逐次計算モード: 入力順で即時評価し中間結果を保持（標準電卓型）。
    - 式評価モード: 括弧・演算子優先でまとめて評価。`=` 押下時に式全体を評価し、エラー時はメッセージ表示＋式を保持。
- テーマ:
  - ライト/ダーク/高コントラストの切替を提供。
- PWA/オフライン:
...
- 永続化は初期スコープ外（リロードで状態はリセット）。メモリ持ち越しが必要なら localStorage で拡張。
...
- フォーマット/静的解析/型: black, isort, flake8, mypy を CI 実行。

## 今後の拡張候補（スコープ外）
- 履歴表示/再利用、括弧を含む式モード、科学関数（sin, cos, pow 等）。
- テーマ切替（ライト/ダーク）、PWA 化によるオフライン常駐。
