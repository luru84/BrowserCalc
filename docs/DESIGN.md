...
### 式評価モードの仕様（詳細）
- 対応演算子: `+`, `-`, `*`, `/`, `%`（単項）、括弧 `(` `)` をサポート。
- 優先順位: 括弧 > 単項（符号/パーセント/税） > 乗除 > 加減。左結合。
- トークン制約: 数値（整数・小数）、演算子、括弧のみを許可。空白は無視。その他文字があれば構文エラー。
- パーセント: トークン単位の単項演算として 1/100 を掛ける（数値リテラル直後、または括弧表現の後に付与可能）。
...
| SYNTAX | Syntax error | 式評価モードでのトークン/括弧不整合 |
